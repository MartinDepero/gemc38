...
<?php if( !is_user_logged_in() && apply_filters('em_booking_form_show_register_form',true) ): ?>
	...
	<p>
		...
		<input type="text" name="user_name" id="user_name" class="input" required="required" value="<?php if(!empty($_REQUEST['user_name'])) echo esc_attr($_REQUEST['user_name']); ?>" />
	...
	<p>
		...
		<input type="text" name="user_email" id="user_email" class="input" required="required" value="<?php if(!empty($_REQUEST['user_email'])) echo esc_attr($_REQUEST['user_email']); ?>"  />
